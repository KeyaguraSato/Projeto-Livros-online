const url = 'A_Arte_Da_Guerra.pdf'; 
let pdfDoc = null;
let currentPage = 1;
...
const loadTimes = () => {
    const savedTotalTime = localStorage.getItem('livro2_totalActiveTime');
    totalActiveTime = savedTotalTime ? parseInt(savedTotalTime) : 0;
    updateTotalTimeDisplay();

    const savedLastSessionTime = localStorage.getItem('livro2_lastSessionTime');
...
const saveTotalTime = () => {
    localStorage.setItem('livro2_totalActiveTime', totalActiveTime);
};

const saveLastSessionTime = (duration) => {
    localStorage.setItem('livro2_lastSessionTime', duration);
};
